...
var socket = io("http://192.168.1.56:3000");
var ping = 0;
...
function Leader() {
    ...
    this.update = function() {
        ...
        if (this.id == player.id) {
            leader.name = player.username;
            this.pos.x = player.pos.x;
            this.pos.y = player.pos.y;
        } else {
            if (enemies[this.id] != null) {
                this.pos.x = enemies[this.id].pos.x;
                this.pos.y = enemies[this.id].pos.y;
                leader.name = enemies[this.id].username;
            }
        }
    };
    this.draw = function() {
...
socket.on("id", function(info) {
    ...
    server_start_time = Date.now() - (info.time + 50);
...
socket.on("a_pos", function(info) {
    //TODO maybe do something more sophisticated than spamming asteroid objects
...
function draw() {
    ...
    ctx.font = '24px serif';
    ctx.fillStyle = 'white';
    ctx.fillText("Leader: " + leader.name, CANVAS_WIDTH/2 - 50, 20);
